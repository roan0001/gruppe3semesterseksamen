...
export default function EventSlider({ events = [] }) {
  ...
  // State for at holde styr på hvilket kort der er aktivt (for at vise overlay på mobil)
  ...
  // Klik på kortet for at åbne/lukke overlay på mobil, hover for at vise overlay på desktop
  const handleCardClick = (eventId) => {
    setActiveId((prev) => (prev === eventId ? null : eventId));
  };

  return (
    // Desktop: 2 kort pr. slide, Mobile: 1 kort pr. slide
    <div className="w-full max-w-[1200px] mx-auto px-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {events.slice(current * itemsToShow, current * itemsToShow + itemsToShow).map((event) => {
          ...
          return (
            //for mobile: klik på kortet for at åbne overlay, klik igen for at lukke. For desktop: hover for at vise overlay, klik for at åbne link
            <div key={event.id} className="relative aspect-[4/3] w-full group overflow-hidden border border-gray-800 cursor-pointer bg-pink-900" onClick={() => handleCardClick(event.id)}>
              ...
              // Overlay vises ved hover på desktop og ved klik på mobil. "Tap for info" vises kun på mobil når overlay er lukket.
              ...
              {/* Tap-indikator på mobil når overlay er lukket */}
              // Kun vis "Tap for info" når kortet ikke er aktivt, og skjul det på desktop
              {!isActive && (
      ...
      <div className="flex justify-center gap-2 mt-8 md:hidden">
        {Array.from({ length: totalSlides }, (_, i) => (
          <button
            ...
            onClick={() => {
              setCurrent(i);
              //set activeId til null for at lukke eventuelle åbne overlays
              setActiveId(null);
            }}
